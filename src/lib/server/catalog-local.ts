...
const DB_PATH = resolve(process.cwd(), 'data', 'sqlite', 'etso-prueba.sqlite');
const TEXTS_DIR = resolve(process.cwd(), 'data', 'texts');
const SUMMARIES_DIR = resolve(process.cwd(), 'data', 'resumenes');
const BIBLIOGRAPHY_PATH = resolve(
	process.cwd(),
	'data',
	'referencias',
	'bibliografia.json'
...
const parseBibliographySource = ():
	...
	| null => {
	if (!existsSync(BIBLIOGRAPHY_PATH)) return null;

	try {
		const raw = readFileSync(BIBLIOGRAPHY_PATH, 'utf8');
		const parsed = JSON.parse(raw) as BibliographyFileRaw;
...
const containsAuthor = (set: AttributionSet, authorId: string): boolean => {
	if (set.unresolved) return false;
	return set.groups.some((group) => group.members.some((member) => member.authorId === authorId));
};

const confidenceForAuthor = (set: AttributionSet, authorId: string): Confidence[] => {
...
const createSnapshot = (): Snapshot => {
	...
	try {
		...
		const attributionRows = db
			...
			.all() as unknown as AttributionRow[];
		...
		const textAccessRows = db
			...
			.all() as unknown as TextAccessRow[];
		...
		const distanceRows = db
			...
			.all() as unknown as DistanceRowRaw[];
		...
		const works: CatalogWork[] = workRows.map((row) => {
			...
			const localTextPath = join(TEXTS_DIR, `${row.id}.txt`);
			if (Number(row.bicuve) === 1 && existsSync(localTextPath)) {
...
export const listGenres = (): string[] =>
	Array.from(new Set(getSnapshot().works.map((work) => work.genre))).sort((a, b) =>
		a.localeCompare(b)
	);
...
export const getInformeById = (informeId: string): CatalogInforme | undefined => {
	...
	return {
		...
		intro:
			work.result1 ||
			'Informe generado desde el SQLite local para validar visualizacion y flujo de consulta.',
		methodology:
			'Se muestran las distancias cargadas en work_distances del SQLite local. En la version final, esta seccion se alimentara desde Turso y servicios de calculo oficiales del proyecto.',
		conclusion:
			work.result2 ||
...
export const getBicuveById = (bicuveId: string): CatalogBicuve | undefined => {
	...
	const textPath = join(TEXTS_DIR, `${bicuveId}.txt`);
